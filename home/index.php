<!-- Main content -->
<section class="content">
  	<div class="row">
    	<div class="col-xs-4">
	      	<div class="box">
	        	<div class="box-body">
	        		<center>
        			<a href="<?php bloginfo('template_url'); ?>/controller.php?ctr=mensagem&mt=addMensagem"><i class="fa fa-comment-o"></i></a><br>
        			<span>Nova Mensagem</span>
        			</center>
	        	</div>
	        </div>
	    </div>
	    <div class="col-xs-4">
	      	<div class="box">
	        	<div class="box-body">
	        		<center>
        			<a href="<?php bloginfo('template_url'); ?>/controller.php?ctr=respostaMsg"><i class="fa fa-inbox"></i></a><br>
        			<span>Mensagens Recebidas</span>
        			</center>
	        	</div>
	        </div>
	    </div>
	    <div class="col-xs-4">
	      	<div class="box">
	        	<div class="box-body">
	        		<center>
        			<a href="<?php bloginfo('template_url'); ?>/controller.php?ctr=pessoas&mt=addPessoas"><i class="fa fa-user"></i></a><br>
        			<span>Cadastrar Pessoas</span>
        			</center>
	        	</div>
	        </div>
	    </div>
	</div>
  	<div class="row">
    	<div class="col-xs-4">
	      	<div class="box">
	        	<div class="box-body">
	        		<center>
        			<a href="https://www.youtube.com/watch?v=QbupTnE_-sg" target="blank"><i class="fa fa-play-circle-o"></i></a><br>
        			<span>V&iacute;deo</span>
        			</center>
	        	</div>
	        </div>
	    </div>
	    <div class="col-xs-4">
	      	<div class="box">
	        	<div class="box-body">
	        		<center>
        			<a href="http://smssocial.gi.fic.ufg.br/tutoriais/" target="blank"><i class="fa fa-book"></i></a><br>
        			<span>Manual do Usu&aacute;rio</span>
        			</center>
	        	</div>
	        </div>
	    </div>
	    <div class="col-xs-4">
	      	<div class="box">
	        	<div class="box-body">
	        		<center>
        			<a href="#"><i class="fa fa-dollar"></i></a><br>
        			<span>Relatório de Cr&eacute;ditos</span>
        			</center>
	        	</div>
	        </div>
	    </div>
	</div>
	<div class="row">
    	<div class="col-xs-6">
	      	<div class="box">
	        	<div class="box-body">
	        		<center>
        			<a href="<?php bloginfo('template_url'); ?>/controller.php?ctr=relatorio&mt=relatorioMsgEnviada" title="Mensagem Enviada" ><i class="fa fa-bar-chart"></i></a><br>
        			<span>Relatório de Mensagens Enviadas</span>
        			</center>
	        	</div>
	        </div>
	    </div>
	    <div class="col-xs-6">
	      	<div class="box">
	        	<div class="box-body">
	        		<center>
        			<a href="<?php bloginfo('template_url'); ?>/controller.php?ctr=relatorio&mt=relatorioMsgRecebida" title="Mensagem Recebida" ><i class="fa fa-line-chart"></i></a><br>
        			<span>Relatório de Mensagens Recebidas</span>
        			</center>
	        	</div>
	        </div>
	    </div>
	</div>
</section>